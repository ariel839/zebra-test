...
- **Design canvas is exactly 1920×1080.** Build to those coordinates. Do **not** invent responsive breakpoints. Fit to smaller screens with the `ScaleToFit` transform wrapper only (Task 3, decided).
- **`position: fixed` is banned inside the canvas.** A CSS `transform` on an ancestor makes `fixed` resolve against the transformed element, so overlays, modals, dropdown panels and tooltips must be `position: absolute` within the 1920×1080 canvas. This is the single most likely source of "why is the modal in the wrong place".
